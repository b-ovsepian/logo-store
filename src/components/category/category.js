...
cartClear.addEventListener('click', e => {
  ...
  if (e.target.name === 'household' || e.target.id === 'household') {
    //Крупная бытовая техника
    cartClear.innerHTML = '';
    renderHouse();
  }
  if (e.target.name === 'embedded' || e.target.id === 'embedded') {
    //Встраиваемая техника
    cartClear.innerHTML = '';
    renderEmbedded();
  }
  if (e.target.name === 'home' || e.target.id === 'home') {
    //Уход за домом и одеждой
    cartClear.innerHTML = '';
    renderHome();
    for (let i = 0; i < 6; i++) {
  ...
  if (e.target.name === 'kitchen' || e.target.id === 'kitchen') {
    //Техника для кухни
    cartClear.innerHTML = '';
    renderKitchen();
  }
...
function renderHouse() {
  getCategories.getCategories().then(data => house(data));
}
function renderEmbedded() {
  getCategories.getCategories().then(data => embed(data));
}
function renderHome() {
  getCategories.getCategories().then(data => hom(data));
}
function renderKitchen() {
  getCategories.getCategories().then(data => kitche(data));
}
